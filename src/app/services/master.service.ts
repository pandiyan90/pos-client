...
export class MasterService {
  ...
  RemoveProduct(slno: number) {
    this.productlist.update((previous) =>
      previous.filter((item) => item.slno !== slno),
    );
  }

  GetProductbyCode(slno: number) {
    this.productitem.set(
      this.productlist().find((item) => item.slno === slno) as SalesProduct,
    );
  }

  totalqty = computed(() => this.productlist().length);
  summarytotal = computed(() =>
    this.productlist().reduce((prev: any, curr: SalesProduct) => {
